angular.module('ghDateRangeService', [])

.service('getDropdownValues', function(){
  return {

    /*-- getting two values to filtering date type*/
     getDatesPair : function(name, tense, single) {
      var result = {
        start_date: null,
        end_date: null
      };
      var date = new Date();
      var getPastFutureDatesPair = function(argName, argTense) {
        var sign = null;
        ...
        switch (argName) {
          ...
          case '30_days_future':
            ...
            break;
          case 'week_past':
            date = new Date(date.getFullYear(), date.getMonth(), date.getDate() - date.getDay() - 1 - 7);
            const endDateWeek = new Date(date.getFullYear(), date.getMonth(), date.getDate() + 7);
            result.end_date = endDateWeek;
            break;
          case 'week_future':
            date = new Date(date.getFullYear(), date.getMonth(), date.getDate() - date.getDay() - 1 + 7);
            result.start_date = date;
          ...
          case 'month_past':
            ...
            const endDateMonth = new Date(date.getFullYear(), date.getMonth() + 1, );
            result.end_date = endDateMonth;
            break;
          case 'month_future':
            date = new Date(date.getFullYear(), date.getMonth() + 1);
            result.start_date = date;
      ...
      var getCurrentDate = function (argName, argTense){
    ...
    getDateDropdownOptions: function () {
      ...
      angular.forEach(options_catalog, (item) => {
        var resultItem = {
